...
export default function ProjectMedia({
  ...
  // En desktop incluimos el cover como primera imagen del carrusel.
  // En móvil NO hace falta (no hay lightbox) y además ocultamos el cover grande.
  const galleryImages = isMobile ? images : [cover, ...(images ?? [])];

  return (
    <>
      {/* COVER clicable: oculto en móvil (hidden sm:block) */}
      <button
        type="button"
        onClick={() => ref.current?.openAt(0)}
        onKeyDown={(e) => (e.key === "Enter" || e.key === " ") && ref.current?.openAt(0)}
        className="group mt-6 relative aspect-[16/9] rounded-2xl overflow-hidden border w-full cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-white/70 hidden sm:block"
      ...
      {/* Miniaturas + Lightbox (o grid estática en móvil) */}
      <GalleryLightbox ref={ref} images={galleryImages} title={title} />
    </>
  );
}
